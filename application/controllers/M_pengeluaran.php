...
class M_pengeluaran extends CI_Controller {
	...
	{
        $data['judul'] = 'Master Pengeluaran';
        $data['konten'] = $this->load->view('admin/m_pengeluaran', $data, true);
	...
	$query_total = $this->Modelku->select($select,'m_pengeluaran a',NULL,NULL,NULL,NULL,$where);
	$query_filter = $this->Modelku->select($select,'m_pengeluaran a',NULL,$where_like,$order,NULL,$where);
	$query = $this->Modelku->select($select,'m_pengeluaran a',$limit,$where_like,$order,NULL,$where);
	$response['data'] = array();
	if ($query<>false) {
		$no = $limit['start']+1;
    ...
    public function tambah() {
        if($this->input->post('id') > 0) {
            ...
            $update = $this->Modelku->update('m_pengeluaran', $data, $where);
            if($update<>false) {
                $response['status'] = '200';
            ...
            $data = $this->general_input_post();
            $insert = $this->db->insert('m_pengeluaran', $data);
            if($insert<>false) {
                $response['status'] = '200';
	...
	public function edit_data($id) {
		$query = $this->db->query("SELECT * FROM m_pengeluaran WHERE id = $id AND status_delete = 0")->row();
        $response['value'] = [
            'id' => $query->id,
            'nama' => $query->nama
	...
	public function delete_data($id) {
		...
		$delete = $this->Modelku->update('m_pengeluaran', $data, $where);
		if($delete<>false) {
			$response['status'] = '200';
