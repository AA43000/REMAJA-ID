...
class Jenis_kegiatan extends CI_Controller {
	...
	public function index()
	{
        ...
        $data['konten'] = $this->load->view('admin/jenis_kegiatan', $data, true);
	...
	$query_total = $this->Modelku->select($select,'jenis_kegiatan a',NULL,NULL,NULL,NULL,$where);
	$query_filter = $this->Modelku->select($select,'jenis_kegiatan a',NULL,$where_like,$order,NULL,$where);
	$query = $this->Modelku->select($select,'jenis_kegiatan a',$limit,$where_like,$order,NULL,$where);
	$response['data'] = array();
	if ($query<>false) {
		$no = $limit['start']+1;
    ...
    public function tambah() {
        if($this->input->post('id') > 0) {
            ...
            $update = $this->Modelku->update('jenis_kegiatan', $data, $where);
            if($update<>false) {
                $response['status'] = '200';
            ...
            $data = $this->general_input_post();
            $insert = $this->db->insert('jenis_kegiatan', $data);
            if($insert<>false) {
                $response['status'] = '200';
	...
	public function edit_data($id) {
		$query = $this->db->query("SELECT * FROM jenis_kegiatan WHERE id = $id AND status_delete = 0")->row();
        $response['value'] = [
            'id' => $query->id,
            'nama' => $query->nama,
	...
	public function delete_data($id) {
		...
		$delete = $this->Modelku->update('jenis_kegiatan', $data, $where);
		if($delete<>false) {
			$response['status'] = '200';
