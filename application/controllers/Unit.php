...
class Unit extends CI_Controller {
	...
	{
        $data['judul'] = 'Unit';
        $data['konten'] = $this->load->view('admin/unit', $data, true);
	...
	$query_total = $this->Modelku->select($select,'unit a',NULL,NULL,NULL,NULL,$where);
	$query_filter = $this->Modelku->select($select,'unit a',NULL,$where_like,$order,NULL,$where);
	$query = $this->Modelku->select($select,'unit a',$limit,$where_like,$order,NULL,$where);
	$response['data'] = array();
	if ($query<>false) {
		$no = $limit['start']+1;
    ...
    public function tambah() {
        if($this->input->post('id') > 0) {
            ...
            $update = $this->Modelku->update('unit', $data, $where);
            if($update<>false) {
                $response['status'] = '200';
	...
	public function edit_data($id) {
		$query = $this->db->query("SELECT * FROM unit WHERE id = $id AND status_delete = 0")->row();
        $response['value'] = [
            'id' => $query->id,
            'nama' => $query->nama,
			'merk' => $query->merk,
	...
	public function delete_data($id) {
		...
		$delete = $this->Modelku->update('unit', $data, $where);
		if($delete<>false) {
			$response['status'] = '200';
